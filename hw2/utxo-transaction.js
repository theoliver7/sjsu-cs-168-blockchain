...
module.exports = class UtxoTransaction extends Transaction {
    ...
    totalInput(block) {
        ...
        for (let address of this.from) {
            inputAmount += block.balanceOf(address)
        }
        return inputAmount
    }
    ...
    validSignature() {
        ...
        for (let address in this.from) {
            if (!utils.addressMatchesKey(this.from[address], this.pubKey[address])) {
                console.log(this.from[address])
                throw new Error('address and public key mismatch');
            } else if (!this.sig[address]) {
                throw new Error('wrong signature');
            } else if (!utils.verifySignature(this.pubKey[address], this.id, this.sig[address])) {
