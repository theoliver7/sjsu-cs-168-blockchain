...
function deployContract(url, contractName, from, gas=GAS, gasPrice=GAS_PRICE_IN_ETH) {
    ...
    let bytecode = fs.readFileSync(`${contractName}_sol_${contractName}.bin`).toString();
    let abi = JSON.parse(fs.readFileSync(`${contractName}_sol_${contractName}.abi`).toString());
...
let contractName = "eVoting";
let fromAddress = "0x2aF1408aAACf96fD117eeBc148B11767B354cb37";
