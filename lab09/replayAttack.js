...
maleficent.on(Blockchain.POST_TRANSACTION, (tx) => {
    ...
    if (tx.outputs[0].address === maleficent.address && !maleficent.oldTransaction) {
        ...
        maleficent.log("*** Recording for replay " + tx.id);
    }
});
...
maleficent.on(Blockchain.PROOF_FOUND, (block) => {
    ...
    if (block.address === minnie.address) {
        maleficent.log("*** Attempting to steal reward")
        if (maleficent.oldTransaction) {
            maleficent.net.broadcast(Blockchain.POST_TRANSACTION, maleficent.oldTransaction)
        }
    }
});
