...
const {Miner, Block} = require('spartan-gold');
...
module.exports = class Validator extends Miner {
    ...
    prevote() {
        ...
        if (this.lockedBlock !== undefined) {
            ...
            let tmpProposal = undefined
            for (let proposal of this.proposals) {
                if (proposal.isValid(this) && tmpProposal === undefined) {
                    tmpProposal = proposal
                } else if (proposal.isValid(this) && proposal.id === tmpProposal.id) {
                    //nop
                } else if (tmpProposal !== undefined) {
                    this.postEvidenceTransaction(proposal.from, tmpProposal, proposal);
                }
            }
            if (undefined !== tmpProposal) {
                vote = Vote.makeVote(this, StakeBlockchain.PREVOTE, tmpProposal.blockID);
            } else {
    ...
    precommit() {
        ...
        if (winningBlockID !== undefined && winningBlockID !== StakeBlockchain.NIL) {
            ...
            this.net.broadcast(StakeBlockchain.PRECOMMIT, Vote.makeVote(this, StakeBlockchain.PRECOMMIT, winningBlockID));
    ...
    commitDecision() {
        ...
        if (undefined !== winningBlockID && StakeBlockchain.NIL !== winningBlockID) {
    ...
    commit(winningBlockID) {
        ...
        this.net.broadcast(StakeBlockchain.COMMIT, Vote.makeVote(this, StakeBlockchain.COMMIT, winningBlockID));
