...
class Mnemonic {
    ...
    constructor(words) {
        ...
        if (words === undefined) {
            ...
            // end of the buffer.
            this.seq.writeUInt8(checkSum, 32)
        } else {
    ...
    words() {
        ...
        arr.forEach(x => {
            passPhrase += this.wordlist[x] + " "
        })
        passPhrase = passPhrase.slice(0, passPhrase.length - 1)
        return passPhrase
    }
    ...
    calculateSequence(words) {
        ...
        for (let bit = 0; bit < bits.length; bit++) {
            if (bytes[bit - 8] !== undefined) {
                bytes[bit - 8] += bits[bit];
            } else {
                bytes[bit - 8] = bits[bit];
            }
        }
        bytes.forEach(x => {
                this.seq.writeUInt8(this.constructor.convertBinStringToByte(bytes[x]), parseInt(byte));
            }
        )
    }
